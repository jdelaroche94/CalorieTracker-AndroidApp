...
/**
 * This class is responsible to create the link and sign the messages between Calorie Tracker and
 * the FatSecretAPI
 * @author FatSecretAPI and Julian De La Roche
 */
public class FatSecretAPI {
    private static final String URL_BASE = "https://platform.fatsecret.com/rest/server.api?";
    ...
    /**
     * This method is responsible to connect with the FatSecretAPI and obtain 5 foods searched by
     * an item.
     * @param item A String value with the item to be searched.
     * @return A String with a JSON object which contains the information searched.
     * @throws FatSecretException A exception if the communication with the FatSecretAPI fails.
     * @author Julian De La Roche
     */
    public String getFood(String item) throws FatSecretException {
        String urlBase = URL_BASE + "max_results=5&method=foods.search&search_expression=" + item + "&format=json";
        String response = "";
        try{
            OAuthBase oAuth = new OAuthBase();
            URL url = new URL(urlBase);
            Result result = new Result();
            oAuth.generateSignature(url, _consumerKey, _consumerSecret, null, null, result);
            response = doHttpMethodReq(result.getNormalizedUrl(), "GET", result.getNormalizedRequestParameters() + "&" + OAuthBase.OAUTH_SIGNATURE + "=" + URLEncoder.encode(result.getSignature(), "utf-8"), null);
        }
    ...
    /**
     * This method is responsible to create a list of food from a JSON object.
     * @param result A String which contains a JSON object with the food found.
     * @param foodListInServer A String with the food which is in the server
     * @return A list of new food.
     * @throws JSONException A exception if the conversion to JSON Object fails.
     * @author Julian De la Roche
     */
    public static ArrayList<Food> getInfoAboutFood (String result, ArrayList<Food> foodListInServer) throws JSONException {
        ...
        try{
            ...
            jsonArray = jsonObject.getJSONObject("foods").getJSONArray("food");
        }catch (JSONException e) {
            e.printStackTrace();
        }
        for(int i=0; i<jsonArray.length(); i++){
            try {
                String foodName = jsonArray.getJSONObject(i).getString("food_name").trim();
                if (foodName.length()>50)
                    foodName = foodName.substring(0,49);
                if (!Input.isItemInListOfFood(foodName,foodListInServer)) {
                    category = new Category((long) i, jsonArray.getJSONObject(i).getString("food_type").trim());
                    String foodDescription = jsonArray.getJSONObject(i).getString("food_description").trim();
                    String foodDetail[] = foodDescription.split("\\|");
                    String infoCalServing[] = foodDetail[0].split("\\-");
                    String servingDetail[] = infoCalServing[0].split("\\ ");
                    String calorieAmount = Input.captureNumberFromString(infoCalServing[1].split("\\:")[1]);
                    String fat = Input.captureNumberFromString(foodDetail[1].split("\\.")[0]);
                    if (servingDetail.length <= 2)
                        serving = new Serving((long) i, "each");
                    else
                        serving = new Serving((long) i, servingDetail[2].trim());
                    foodList.add(new Food((long) i, foodName, category, Integer.parseInt(calorieAmount.trim()),
                            serving, new BigDecimal(1), Integer.parseInt(fat.trim())));
    ...
    private String doHttpMethodReq(String urlStr, String requestMethod, String paramStr, Map<String, String> header) throws FatSecretException{
        ...
        String urlBase = urlStr + "?" + paramStr;
        URL url = null;
    ...
    private static class OAuthBase{
        ...
        public void generateSignature(URL url, String consumerKey, String consumerSecret, String token, String tokenSecret, Result result) {
            GenerateSignatureBase(url, consumerKey, token, "GET", generateTimeStamp(), generateNonce(), "HMAC-SHA1", result);
        ...
        private void GenerateSignatureBase(URL url, String consumerKey, String token, String httpMethod, String timeStamp, String nonce, String signatureType, Result result){
